...
if (empty($token)) {
    header('Location: /login.php?message=' . urlencode('Invalid reset link') . '&type=error');
    exit;
}
...
if (!$validToken) {
    header('Location: /login.php?message=' . urlencode('Reset link is invalid or expired') . '&type=error');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrfToken($_POST['csrf_token'] ?? '')) {
    ...
    if (empty($errors)) {
        ...
        if ($result['success']) {
            header('Location: /login.php?message=' . urlencode('Password reset successfully! You can now login.') . '&type=success');
            exit;
        } else {
